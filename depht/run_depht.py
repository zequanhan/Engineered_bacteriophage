...
#sys.path.append('/home/hanzequan/miniconda3/envs/depht/lib/python3.9/site-packages/depht')
...
def parse_args():
    ...
    p.add_argument("file_path",  type=pathlib.Path,
                   help="DBSACN_SWA analysis result path")
    # Optional arguments
...
def main():
    ...
    #infiles = '/home/hanzequan/test_bectiral/depht_test/CP082203.1/CP082203.1.gbk'
    #outdir = '/home/hanzequan/test_bectiral/depht_test'
    ...
    for infile in infiles:
        ...
        for i, contig in enumerate(contigs):
            ...
            #prediction = [(116509,159646),(617550,628355),(662225,679673),(1191792,1235754),(1241088,1253149,),(1420069,1436127),(1439765,1458497),(2154489,2161116)]
            #prediction = [(31438,41700),(542568,555688),(636598,651943),(1209392,1215177),(1364975,1375017)]           
            prediction = extend_prophage_coords(file_path)##2024.1.22修改
            print('prediction:',prediction)
            filtered_prediction = []
            for pred in prediction:
                if len(range(*pred)) < 2000:#min_length从2000改为2000  2024.1.20
...
def write_prophage_metadata(outdir, prophages):
    ...
    handle.close()
def extend_prophage_coords(file_path, extension=5000,sep='\t'):
    """
    ...
    if 'prophage_start' not in data.columns or 'prophage_end' not in data.columns:
        raise ValueError("Required columns 'prophage_start' and 'prophage_end' are not present in the data")

    # Extend coordinates
    extended_coords = []
    for _, row in data.iterrows():
        # Extend start and end, but ensure they don't go below zero
        start = max(0, row['prophage_start'] - extension)
        end = row['prophage_end'] + extension  # Assuming no upper boundary is specified
        extended_coords.append((start, end))
